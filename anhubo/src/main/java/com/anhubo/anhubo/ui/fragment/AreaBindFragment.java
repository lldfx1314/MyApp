 ...
 */
public class AreaBindFragment extends BaseFragment implements QRCodeView.Delegate{
    private static final String TAG = "AreaBindFragment";
    ...
    public Object getContentView() {
        return R.layout.areabinding_fragment;
    }

    @Override
    public void initView() {
        llTop.setVisibility(View.GONE);
        mQRCodeView = findView(R.id.zxingview_areabind);
        btnNfcScan = findView(R.id.btn_nfcScan_areabind);
        btnCompleteCheck = findView(R.id.btn_completeCheck_areabind);
        btnLight = findView(R.id.btn_light_areabind);
        btnNfcScan.setText("区域绑定");
        // 设置扫描二维码的代理
        mQRCodeView.setDelegate(this);
//        btnCompleteCheck.setText("完成HAHA");
    }
...
//    }
    ...
    private void processData(String cardNumber) {
        if (!TextUtils.isEmpty(cardNumber)) {
            /***********新增**************************/
//            addNewDevice();
            ToastUtils.showToast(mActivity,"这是区域绑定");
        }
    }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.btn_nfcScan_areabind:
                // 跳转到区域绑定界面
                jumpAreaBinding();
                break;
            case R.id.btn_completeCheck_areabind://完成检查
//                mActivity.finish();
                break;
            case R.id.btn_light_areabind:
                // 开关灯
                if (!isLight) {
